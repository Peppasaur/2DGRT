...
import numpy as np
import torch
#print("gtracer is installed at:", os.path.dirname(gtracer.__file__))
...
class _GaussianTrace(torch.autograd.Function):
    ...
    def forward(ctx, bvh, rays_o, rays_d, gs_idxs, means3D, opacity, SinvR, shs, alpha_min, transmittance_min, deg):    
        ...
        alpha = torch.zeros((3+3+1, rays_o.size(0)), dtype=torch.float32, device=device)
        depth = torch.zeros((means3D.size(0)), dtype=torch.float32, device=device)
        #print("alpha")
        #print(alpha.shape)
    ...
    def backward(ctx, grad_out_color, grad_out_depth, grad_out_alpha):
        ...
        grad_out_depth = torch.zeros_like(rays_o[:, 0])
        grad_out_alpha = torch.zeros_like(rays_o[:, 0])
