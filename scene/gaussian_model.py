...
class GaussianModel:
    ...
    def load_ply2(self, path, use_train_test_exp = False):
    ...
    def load_ply(self, path):
        ...
        zeroline=torch.full((scales.shape[0],1),0)
        scaling = torch.tensor(scales)[:,:2]
        
        scaling=torch.cat((scaling,zeroline),dim=1)
        #scaling+=2.5
        print(rots)
        #opacities*=0.000001

        lim=500000
        self._xyz = nn.Parameter(torch.tensor(xyz[:lim,:], dtype=torch.float, device="cuda").requires_grad_(True))
        self._features_dc = nn.Parameter(torch.tensor(features_dc[:lim,:,:], dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(torch.tensor(features_extra[:lim,:,:], dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(torch.tensor(opacities[:lim,:], dtype=torch.float, device="cuda").requires_grad_(True))
        self._scaling = nn.Parameter(torch.tensor(scaling[:lim,:], dtype=torch.float, device="cuda").requires_grad_(True))
        self._rotation = nn.Parameter(torch.tensor(rots[:lim,:], dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def get_bound_surfel(self,alpha_min=0.01):
        scaling=self.get_scaling
        scaling[:,2]=0
        L = build_scaling_rotation(scaling, self._rotation)
        opacity = self.get_opacity
        vertices=torch.empty((self.get_scaling.size(0),4,3),device="cuda")
        vertices[:, :, :] =1
        vertices[:, :, 2] =0
        vertices[:, 1, 0]=-1
        vertices[:, 2, 1]=-1
        vertices[:, 3, :]=-1
        L_mat=L.reshape(-1,1,3,3)
        v_mat=vertices.reshape(-1,4,3,1)
        nv=torch.matmul(L_mat,v_mat)
        nv*=(3 * (opacity/alpha_min).log()).sqrt()[:, :,None,None]
        nv=nv.squeeze(3)
        
        nv=nv+self.get_xyz[:,None,:]
        
        faces=torch.zeros((self.get_scaling.size(0),2,3),device="cuda", dtype=torch.int64)
        faces+=torch.arange(self.get_scaling.size(0), device="cuda")[:,None,  None]*4
        print(faces.shape)
        faces[:,0,1]+=1
        faces[:,0,2]+=2
        faces[:,1,0]+=3
        faces[:,1,1]+=1
        faces[:,1,2]+=2
        gs_id = torch.arange(opacity.shape[0], device="cuda")[:, None].expand(-1, faces.shape[1])

        return nv,faces,gs_id
    ...
    def get_boundings(self, alpha_min=0.01):
        ...
        '''
        vertices_b,faces_b,gs_id=self.get_bound_surfel()
        #print("vertices_b")
        #print(vertices_b)
        '''
        print("faces_b")
    ...
    def get_SinvR(self):
        ...
        #return build_scaling_rotation(self.get_scaling, self._rotation)
